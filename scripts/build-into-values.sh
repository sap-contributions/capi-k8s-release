...
IMAGE_DESTINATION_CAPI_NGINX="${IMAGE_DESTINATION_CAPI_NGINX:-gcr.io/cf-capi-arya/dev-capi-nginx}"
...
CAPI_NGINX_DIR="${REPO_BASE_DIR}/dockerfiles/nginx"
...
ytt -f "${REPO_BASE_DIR}/dev-templates/" \
    ...
    -v kbld.destination.capi_nginx="${IMAGE_DESTINATION_CAPI_NGINX}" \
    -v src_dirs.ccng="${CCNG_DIR}" \
    -v src_dirs.capi_kpack_watcher="${CAPI_KPACK_WATCHER_DIR}" \
    -v src_dirs.capi_nginx="${CAPI_NGINX_DIR}" \
     > "${KBLD_TMP}"
